  ...
  onStart: async function ({
    ...
    if (args[0] == "reset") {
      ...
      try {
        ...
        // Change nickname immediately for the bot itself
        api.changeNickname(defaultNickname, event.threadID, botUserID);

        // Send the reset message
        const resetMessage = getLang("reset", global.GoatBot.config.prefix);
        return message.reply(resetMessage);
      } catch (error) {
        console.error(
          "An error occurred while getting the bot's user ID:",
          error
        );
