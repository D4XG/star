...
module.exports = async function (databaseType, userModel, api, fakeGraphql) {
  ...
  switch (databaseType) {
    ...
    case "sqlite": {
      Users = (await userModel.findAll()).map((user) =>
        user.get({ plain: true })
      );
      break;
    }
        ...
        case "update": {
          ...
          switch (databaseType) {
            ...
            case "sqlite": {
              ...
              const dataUpdated = (await user.update(dataWillChange)).get({
                plain: true,
              });
              global.db.allUserData[index] = dataUpdated;
              return _.cloneDeep(dataUpdated);
  ...
  async function getName(userID, checkData = true) {
    ...
    if (checkData) return getNameInDB(userID);
  ...
  async function create_(userID, userInfo) {
    return new Promise(async function (resolve, reject) {
      const findInCreatingData = creatingUserData.find(
        (u) => u.userID == userID
      );
      ...
      const queue = new Promise(async function (resolve_, reject_) {
        ...
        creatingUserData.splice(
          creatingUserData.findIndex((u) => u.userID == userID),
          1
        );
      });
        ...
        try {
          ...
          if (query)
            if (typeof query !== "string")
              throw new Error(
                `The third argument (query) must be a string, not ${typeof query}`
              );
            else
          ...
          if (path)
            if (!["string", "object"].includes(typeof path))
              throw new Error(
                `The first argument (path) must be a string or object, not ${typeof path}`
              );
            else if (typeof path === "string")
              return resolve(
                dataReturn.map((uData) => _.get(uData, path, defaultValue))
              );
            else
              return resolve(
                dataReturn.map((uData) =>
                  _.times(path.length, (i) =>
                    _.get(uData, path[i], defaultValue[i])
                  )
                )
              );
  ...
  async function get_(userID, path, defaultValue, query) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const index = global.db.allUserData.findIndex(
          (u) => u.userID == userID
        );
        ...
        if (query)
          if (typeof query !== "string")
            throw new Error(
              `The fourth argument (query) must be a string, not ${typeof query}`
            );
        ...
        if (path)
          if (!["string", "array"].includes(typeof path))
            throw new Error(
              `The second argument (path) must be a string or array, not ${typeof path}`
            );
          else if (typeof path === "string")
            ...
            return resolve(
              _.cloneDeep(
                _.times(path.length, (i) =>
                  _.get(userData, path[i], defaultValue[i])
                )
              )
            );
      ...
      messageQueue.push(async function () {
        try {
          ...
          if (
            ...
            throw new Error(
              `The second argument (updateData) must be an object, not ${typeof updateData}`
            );
          ...
          if (query)
            ...
            else return resolve(_.cloneDeep(fakeGraphql(query, userData)));
    ...
    return new Promise(async function (resolve, reject) {
      messageQueue.push(async function () {
        try {
          ...
          if (!parentData)
            throw new Error(
              `Can't find key "${parent}" in user with userID: ${userID}`
            );
        ...
        try {
          if (isNaN(userID)) {
            const error = new Error(
              `The first argument (userID) must be a number, not ${typeof userID}`
            );
