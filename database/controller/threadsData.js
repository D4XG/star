...
module.exports = async function (databaseType, threadModel, api, fakeGraphql) {
  ...
  switch (databaseType) {
    case "mongodb": {
      ...
      Threads = (await threadModel.find({}).lean()).map((thread) =>
        _.omit(thread, ["_id", "__v"])
      );
      break;
    }
    case "sqlite": {
      Threads = (await threadModel.findAll()).map((thread) =>
    ...
    try {
      ...
      switch (mode) {
        ...
        case "update": {
          ...
          switch (databaseType) {
            case "mongodb": {
              let dataUpdated = await threadModel.findOneAndUpdate(
                { threadID },
                dataWillChange,
            ...
            case "sqlite": {
              ...
              const dataUpdated = (await thread.update(dataWillChange)).get({
                plain: true,
              });
              global.db.allThreadData[index] = dataUpdated;
              return _.cloneDeep(dataUpdated);
  ...
  async function create_(threadID, threadInfo) {
    return new Promise(async function (resolve, reject) {
      const findInCreatingData = creatingThreadData.find(
        (t) => t.threadID == threadID
      );
      if (findInCreatingData) return resolve(findInCreatingData.promise);
      ...
      const queue = new Promise(async function (resolve_, reject_) {
        ...
        creatingThreadData.splice(
          creatingThreadData.findIndex((t) => t.threadID == threadID),
          1
        );
      });
        ...
        try {
          ...
          for (const user of userInfo) {
            ...
            const data = {
              ...
              permissionConfigDashboard:
                oldDataUser.permissionConfigDashboard || false,
            };
            indexUser != -1
              ? (oldMembers[indexUser] = data)
              : oldMembers.push(data);
            newMembers.push(
              oldMembers.splice(
                indexUser != -1 ? indexUser : oldMembers.length - 1,
                1
              )[0]
            );
          }
        ...
        try {
          ...
          if (query)
            if (typeof query !== "string")
              throw new Error(
                `The third argument (query) must be a string, not a ${typeof query}`
              );
            else
          ...
          if (path)
            if (!["string", "object"].includes(typeof path))
              throw new Error(
                `The first argument (path) must be a string or an object, not a ${typeof path}`
              );
            else if (typeof path === "string")
              return resolve(
                dataReturn.map((tData) => _.get(tData, path, defaultValue))
              );
            else
              return resolve(
                dataReturn.map((tData) =>
                  _.times(path.length, (i) =>
                    _.get(tData, path[i], defaultValue[i])
                  )
                )
              );
  ...
  async function get_(threadID, path, defaultValue, query) {
    return new Promise(async function (resolve, reject) {
      try {
        ...
        const index = global.db.allThreadData.findIndex(
          (t) => t.threadID == threadID
        );
        ...
        if (query)
          if (typeof query != "string")
            throw new Error(
              `The fourth argument (query) must be a string, not a ${typeof query}`
            );
          else threadData = fakeGraphql(query, threadData);

        if (path)
          if (!["string", "object"].includes(typeof path))
            throw new Error(
              `The second argument (path) must be a string or an object, not a ${typeof path}`
            );
          else if (typeof path === "string")
            ...
            return resolve(
              _.cloneDeep(
                _.times(path.length, (i) =>
                  _.get(threadData, path[i], defaultValue[i])
                )
              )
            );
      ...
      messageQueue.push(async function () {
        try {
          ...
          if (
            ...
            throw new Error(
              `The second argument (updateData) must be an object, not a ${typeof updateData}`
            );
          ...
          if (query)
            ...
            else return resolve(_.cloneDeep(fakeGraphql(query, threadData)));
          return resolve(_.cloneDeep(threadData));
        } catch (err) {
          reject(err);
        }
      ...
      messageQueue.push(async function () {
        try {
          ...
          if (typeof path !== "string")
            throw new Error(
              `The second argument (path) must be a string, not a ${typeof path}`
            );
          const spitPath = path.split(".");
          if (spitPath.length == 1)
            throw new Error(
              `Can't delete key "${path}" because it's a root key`
            );
          const parent = spitPath.slice(0, spitPath.length - 1).join(".");
          const parentData = await get_(threadID, parent);
          if (!parentData)
            throw new Error(
              `Can't find key "${parent}" in thread with threadID: ${threadID}`
            );
          ...
          if (query)
            ...
            else return resolve(_.cloneDeep(fakeGraphql(query, setData)));
